...
SELECTED_FIELDS = ['chol', 'restecg', 'oldpeak', 'ca']
...
def evaluate_uploaded_csv(file_path):
    ...
    for name, path in MODELS.items():
        model = joblib.load(path)
        X_sel = X[SELECTED_FIELDS].copy()
        X_sel = pd.DataFrame(X_sel, columns=SELECTED_FIELDS)
        if name == "XGBoost":
            y_pred = model.predict(X_sel.values)
        else:
...
def manual():
    ...
    if request.method == 'POST':
        values = np.array([float(request.form[f]) for f in SELECTED_FIELDS]).reshape(1, -1)
    ...
    return render_template("manual1.html", result=combined_result, votes=votes)
